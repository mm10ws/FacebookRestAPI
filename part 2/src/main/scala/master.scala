...
class MasterActor(numClients: Int, numTasks: Int) extends Actor {
  ...
  for (i <- 0 until numClients) {
    masterActorList += context.actorOf(Props(new clientActor(i, friendlists(i).toArray, self, numTasks)), name = "ClientActor" + i)
  }
  ...
  def receive = {
    ...
    case begin() =>
      ...
      for (i <- 0 until numClients) {
        context.system.scheduler.scheduleOnce(5000.milliseconds, masterActorList(i), startActivity2(numTasks))
